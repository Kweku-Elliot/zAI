...
export default function OnboardingPage() {
  ...
  return (
  <div className="min-h-screen bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center mobile-safe-container">
  ...
  <p className="text-muted-foreground text-center mb-8">
        ...
        <div className="flex gap-2 mb-6">
          {onboardingData.map((_, idx) => (
            <span
              key={idx}
              className={`w-3 h-3 rounded-full ${idx === current ? 'bg-primary' : 'bg-muted'}`}
            />
          ))}
        </div>
        <div className="flex w-full justify-between">
          <button
            className="px-6 py-2 rounded-xl bg-muted text-muted-foreground font-semibold"
            onClick={prev}
            disabled={current === 0}
          >
            Previous
